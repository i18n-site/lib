...
const match = (host_li, target) => {
  for (const d of host_li) {
    if (d === target || (d[0] === "*" && target.endsWith(d.slice(1)))) return true;
  }
  return false;
};
...
const upload = async (cas, name, key, crt) => {
  try {
    return (
      await cas.createUserCertificate(new CreateUserCertificateRequest({ name, key, cert: crt }))
    ).body.certId;
  } catch (e) {
...
const sha256 = async (str) =>
  toHex(new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(str))));
...
const ossSign = async (id, secret, bucket, endpoint, date, hash) => {
  ...
  for (const s of [region, "oss", "aliyun_v4_request"]) key = await hmac(key, s);
...
export default ([id, secret]) => {
  const conf = (endpoint) => new Config({ accessKeyId: id, accessKeySecret: secret, endpoint }),
  ...
  const listCdn = async () => {
    ...
    for await (const d of pageiter((p) =>
      cdn
        .describeUserDomains(new DescribeUserDomainsRequest({ pageNumber: p, pageSize: 500 }))
  ...
  return async ([key, crt], opt) => {
    ...
    for (const d of cdn_li)
      if (match(host_li, d)) tasks.push(wrap("cdn", d, () => bindCdn(d, cert_id)));
    for (const [sid, name] of esa_li)
      if (match(host_li, name)) tasks.push(wrap("esa", name, () => bindEsa(sid, cert_id)));
