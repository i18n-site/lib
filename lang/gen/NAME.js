/*
语言名称映射表
格式: [中文名称, 英文名称, 本地名称]
键: 语言代码 (ISO 639-1 或 变体)
*/
export default new Map([
  ["en", ["英语", "English", "English"]],
  ["zh", ["简体中文", "Chinese", "中文"]],
  ["zh-TW", ["正體中文", "Traditional Chinese", "正體中文"]],
  ["es", ["西班牙语", "Spanish", "Español"]],
  ["fr", ["法语", "French", "Français"]],
  ["ja", ["日语", "Japanese", "日本語"]],
  ["de", ["德语", "German", "Deutsch"]],
  ["ar", ["阿拉伯语", "Arabic", "العربية"]],
  ["it", ["意大利语", "Italian", "Italiano"]],
  ["ko", ["韩语", "Korean", "한국어"]],
  ["ru", ["俄语", "Russian", "Русский"]],
  ["hi", ["印地语", "Hindi", "हिन्दी"]],
  ["nl", ["荷兰语", "Dutch", "Nederlands"]],
  ["tr", ["土耳其语", "Turkish", "Türkçe"]],
  ["id", ["印度尼西亚语", "Indonesian", "Bahasa Indonesia"]],
  ["bn", ["孟加拉语", "Bengali", "বাংলা"]],
  ["pl", ["波兰语", "Polish", "Polski"]],
  ["da", ["丹麦语", "Danish", "Dansk"]],
  ["fi", ["芬兰语", "Finnish", "Suomi"]],
  ["no", ["书面挪威语", "Norwegian Bokmål", "Norsk Bokmål"]],
  ["sv", ["瑞典语", "Swedish", "Svenska"]],
  ["th", ["泰语", "Thai", "ไทย"]],
  ["vi", ["越南语", "Vietnamese", "Tiếng Việt"]],
  ["el", ["希腊语", "Greek", "Ελληνικά"]],
  ["he", ["希伯来语", "Hebrew", "עברית"]],
  ["iw", ["希伯来语", "Hebrew", "עברית"]],
  ["ro", ["罗马尼亚语", "Romanian", "Română"]],
  ["uk", ["乌克兰语", "Ukrainian", "Українська"]],
  ["fa", ["波斯语", "Persian", "فارسی"]],
  ["ta", ["泰米尔语", "Tamil", "தமிழ்"]],
  ["te", ["泰卢固语", "Telugu", "తెలుగు"]],
  ["ur", ["乌尔都语", "Urdu", "اردو"]],
  ["bg", ["保加利亚语", "Bulgarian", "Български"]],
  ["hr", ["克罗地亚语", "Croatian", "Hrvatski"]],
  ["cs", ["捷克语", "Czech", "Čeština"]],
  ["et", ["爱沙尼亚语", "Estonian", "Eesti"]],
  ["hu", ["匈牙利语", "Hungarian", "Magyar"]],
  ["lv", ["拉脱维亚语", "Latvian", "Latviešu Valoda"]],
  ["lt", ["立陶宛语", "Lithuanian", "Lietuvių Kalba"]],
  ["sr", ["塞尔维亚语", "Serbian", "Српски Језик"]],
  ["sk", ["斯洛伐克语", "Slovak", "Slovenčina"]],
  ["sl", ["斯洛文尼亚语", "Slovenian", "Slovenščina"]],
  ["sw", ["斯瓦希里语", "Swahili", "Kiswahili"]],
  ["af", ["南非荷兰语", "Afrikaans", "Afrikaans"]],
  ["as", ["阿萨姆语", "Assamese", "অসমীয়া"]],
  ["az", ["阿塞拜疆语", "Azerbaijani", "Azərbaycanca"]],
  ["be", ["白俄罗斯语", "Belarusian", "Беларуская"]],
  ["bs", ["波斯尼亚语", "Bosnian", "Bosanski"]],
  ["ca", ["加泰罗尼亚语", "Catalan", "Català"]],
  ["ceb", ["宿务语", "Cebuano", "Binisaya"]],
  ["cy", ["威尔士语", "Welsh", "Cymraeg"]],
  ["eu", ["巴斯克语", "Basque", "Euskara"]],
  ["ga", ["爱尔兰语", "Irish", "Gaeilge"]],
  ["gl", ["加利西亚语", "Galician", "Galego"]],
  ["gu", ["古吉拉特语", "Gujarati", "ગુજરાતી"]],
  ["ht", ["海地克里奥尔语", "Haitian Creole", "Kreyòl Ayisyen"]],
  ["hy", ["亚美尼亚语", "Armenian", "Հայերեն"]],
  ["is", ["冰岛语", "Icelandic", "Íslenska"]],
  ["jw", ["爪哇语", "Javanese", "Basa Jawa"]],
  ["jv", ["爪哇语", "Javanese", "Basa Jawa"]],
  ["ka", ["格鲁吉亚语", "Georgian", "ქართული"]],
  ["kk", ["哈萨克语", "Kazakh", "Қазақ Тілі"]],
  ["km", ["高棉语", "Khmer", "ភាសាខ្មែរ"]],
  ["kn", ["卡纳达语", "Kannada", "ಕನ್ನಡ"]],
  ["lb", ["卢森堡语", "Luxembourgish", "Lëtzebuergesch"]],
  ["lo", ["老挝语", "Lao", "ພາສາລາວ"]],
  ["mk", ["马其顿语", "Macedonian", "Македонски Јазик"]],
  ["ml", ["马拉雅拉姆语", "Malayalam", "മലയാളം"]],
  ["mr", ["马拉地语", "Marathi", "मরাठी"]],
  ["mt", ["马耳他语", "Maltese", "Malti"]],
  ["my", ["缅甸语", "Burmese", "မြန်မာစာ"]],
  ["ne", ["尼泊尔语", "Nepali", "नेपाली"]],
  ["or", ["奥里亚语", "Odia", "ଓଡ଼ିଆ"]],
  ["sd", ["信德语", "Sindhi", "سنڌي"]],
  ["si", ["僧伽罗语", "Sinhala", "සිංහල"]],
  ["su", ["巽他语", "Sundanese", "Basa Sunda"]],
  ["tg", ["塔吉克语", "Tajik", "Тоҷикӣ"]],
  ["yi", ["意第绪语", "Yiddish", "ייִדיש"]],
  ["sq", ["阿尔巴尼亚语", "Albanian", "Shqip"]],
  ["am", ["阿姆哈拉语", "Amharic", "አማርኛ"]],
  ["ha", ["豪萨语", "Hausa", "هَوُسَ"]],
  ["ky", ["吉尔吉斯语", "Kyrgyz", "Кыргыз Тили"]],
  ["xh", ["科萨语", "Xhosa", "IsiXhosa"]],
  ["co", ["科西嘉语", "Corsican", "Corsu"]],
  ["ku", ["库尔德语", "Kurdish", "Kurdî"]],
  ["la", ["拉丁语", "Latin", "Lingua Latīna"]],
  ["mg", ["马拉加斯语", "Malagasy", "Malagasy"]],
  ["ms", ["马来语", "Malay", "Bahasa Melayu"]],
  ["mi", ["毛利语", "Maori", "Te Reo Māori"]],
  ["mn", ["蒙古语", "Mongolian", "Монгол Хэл"]],
  ["hmn", ["苗语", "Hmong", "Hmoob"]],
  ["st", ["南索托语", "Sesotho", "Sesotho"]],
  ["pa", ["旁遮普语", "Punjabi", "ਪੰਜਾਬੀ"]],
  ["pt", ["葡萄牙语", "Portuguese", "Português"]],
  ["ps", ["普什图语", "Pashto", "پښتو"]],
  ["ny", ["齐切瓦语", "Nyanja", "ChiCheŵa"]],
  ["sm", ["萨摩亚语", "Samoan", "Gagana Fa'a Samoa"]],
  ["sn", ["绍纳语", "Shona", "ChiShona"]],
  ["eo", ["世界语", "Esperanto", "Esperanto"]],
  ["gd", ["苏格兰盖尔语", "Scots Gaelic", "Gàidhlig"]],
  ["so", ["索马里语", "Somali", "Soomaaliga"]],
  ["uz", ["乌兹别克语", "Uzbek", "O'zbek"]],
  ["fy", ["西弗里西亚语", "Frisian", "Frysk"]],
  ["haw", ["夏威夷语", "Hawaiian", "ʻŌlelo Hawaiʻi"]],
  ["ig", ["伊博语", "Igbo", "Igbo"]],
  ["yo", ["约鲁巴语", "Yoruba", "Yorùbá"]],
  ["zu", ["祖鲁语", "Zulu", "IsiZulu"]],
  ["fil", ["菲律宾语", "Filipino", "Wikang Filipino"]],
  ["dv", ["迪维希语", "Dhivehi", "ދިވެހިބަސް"]],
  ["kri", ["克里奥语", "Krio", "Krio"]],
  ["mni-Mtei", ["梅泰语", "Manipuri", "মৈতেইলোন্"]],
  ["ug", ["维吾尔语", "Uyghur", "ئۇيغۇرچە"]],
  ["yue", ["粤语", "Cantonese", "粵語"]],
  ["ast", ["阿斯图里亚斯语", "Asturian", "Asturianu"]],
  ["arz", ["埃及阿拉伯语", "Egyptian Arabic", "مصرى"]],
  ["mai", ["迈蒂利语", "Maithili", "मैथिली"]],
  ["acm", ["美索不达米亚阿拉伯语", "Mesopotamian Arabic", "العربية العراقية"]],
  ["ary", ["摩洛哥阿拉伯语", "Moroccan Arabic", "الدارجة المغربية"]],
  ["ars", ["内志阿拉伯语", "Najdi Arabic", "العربية النجدية"]],
  ["apc", ["北黎凡特阿拉伯语", "North Levantine Arabic", "اللغة العربية اللبنانية"]],
  ["nn", ["新挪威语", "Nynorsk", "Nynorsk"]],
  ["oc", ["奥克语", "Occitan", "Occitan"]],
  ["pag", ["邦阿西楠语", "Pangasinan", "Salitan Pangasinan"]],
  ["scn", ["西西里语", "Sicilian", "Sicilianu"]],
  ["ajp", ["南黎凡特阿拉伯语", "South Levantine Arabic", "العربية الفلسطينية"]],
  ["acq", ["塔伊兹-亚丁阿拉伯语", "Ta'izzi-Adeni Arabic", "العربية التعزية العدنية"]],
  ["aeb", ["突尼斯阿拉伯语", "Tunisian Arabic", "الدارجة التونسية"]],
  ["vec", ["威尼斯语", "Venetian", "Vèneto"]],
  ["war", ["瓦莱语", "Waray", "Winaray"]],
]);
