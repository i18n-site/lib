...
const rm_cdn_headers = ["age", "date", "last-modified", "nel", "report-to"];
...
const cdn_headers = Object.fromEntries(rm_cdn_headers.map((h) => [h, { operation: "remove" }]));
...
const bind = async (cf, zone_id, host, backblazeb2, bucket) => {
  ...
  new_req_rules.push(
    rule(req_desc, `(http.host in {"${host}"})`, {
  ...
  const new_res_rules = res_rules.filter(
    (r) => r.description !== "rm_b2_head" && r.description !== "删除cdn头",
  );

  new_res_rules.push(
    rule("rm_b2_head", addHostToExpression(b2_expr, host), { headers: b2_headers }),
  );

  new_res_rules.push(rule("删除cdn头", "true", { headers: cdn_headers }));
