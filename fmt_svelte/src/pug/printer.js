...
import { isAngularAction, isAngularBinding, isAngularDirective, isAngularInterpolation, } from './utils/angular.js';
import { detectDangerousQuoteCombination, detectFramework, handleBracketSpacing, isMultilineInterpolation, isQuoted, isSingleLineWithInterpolation, isStyleAttribute, makeString, previousNormalAttributeToken, previousTagToken, previousTypeAttributeToken, unwrapLineFeeds, } from './utils/common.js';
import { getScriptParserName } from './utils/script-mime-types.js';
import { isSvelteInterpolation } from './utils/svelte.js';
import { isVueEventBinding, isVueExpression, isVueVBindExpression, isVueVDirective, isVueVForWithOf, isVueVOnExpression, } from './utils/vue.js';
/**
 * The printer class.
 */
export class PugPrinter {
            ...
            try {
                switch (token.type) {
                    ...
                    default: {
                        ...
                        results.push(await this[token.type](
            ...
            if (start === -1) {
                ...
                if (this.options.pugFramework === 'svelte' && start2 !== -1) {
                    ...
                    if (end2 === -1) {
                        ...
                        try {
                            const dangerousQuoteCombination = detectDangerousQuoteCombination(code, this.quotes, this.otherQuotes, logger);
    ...
    async formatFrameworkInterpolation(val, parser, // TODO: may be changed to allow a special parser for svelte
    [opening, closing]) {
        val = val.slice(1, -1); // Remove quotes
    ...
    ['start-attributes'](token) {
        ...
        if (this.nextToken?.type === 'attribute') {
            ...
            if (hasLiteralAttributes && // Remove div as it will be replaced with the literal for id and/or class
            ...
            if (this.options.pugSortAttributes !== 'as-is' ||
                ...
                if (endAttributesIndex - startAttributesIndex > 2) {
                    this.tokens = partialSort(this.tokens, startAttributesIndex + 1, endAttributesIndex, (a, b) => compareAttributeToken(a, b, this.options.pugSortAttributes, this.options.pugSortAttributesBeginning, this.options.pugSortAttributesEnd));
                }
            }
        }
        return result;
    ...
    async attribute(token) {
        ...
        if (typeof token.val === 'boolean') {
            ...
            this.result += `=${isQuoted(token.val)
    ...
    ['end-attributes'](token) {
        if (this.wrapAttributes && this.result.at(-1) !== '(') {
            ...
            this.result += this.indentString.repeat(this.indentLevel + this.options.pugClosingBracketIndentDepth);
        }
    ...
    class(token) {
        if (this.options.pugClassNotation === 'attribute') {
            ...
            if (this.checkTokenType(this.nextToken, [
                'text',
                'newline',
                'indent',
                'outdent',
        ...
        try {
            ...
            try {
                ...
                */
                const ma = /\s{\n?}(.*)$/.exec(val);
                if (!ma)
                    throw new Error('No follow-up block found');
                const comment = ma[1];
