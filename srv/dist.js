...
const dir = import.meta.dirname,
  ...
  getHash = async (data) =>
    Buffer.from(await crypto.subtle.digest("SHA-256", data)).toString("base64url"),
  changed = [],
  current = {};

try {
  unlinkSync(join(dir, "srv/bun.lock"));
} catch (e) {}
...
for (const pkg of packages) {
  ...
  current[pkg] = {
    hash,
    name: JSON.parse(readFileSync(join(pkg_dir, "package.json"), "utf8")).name,
  };
...
if (changed.length > 0) {
  ...
  for (const { dir: pkg_dir } of changed) {
    ...
    if (has_workspace) {
      const replaced = origin_pkg_json.replace(
        /"(@3-\/srv-(.*?))":\s*"workspace:\*"/g,
        (_, dep, p) =>
          `"${dep}": "${JSON.parse(readFileSync(join(dir, p, "package.json"), "utf8")).version}"`,
      );
  ...
  const { version: v } = JSON.parse(
    readFileSync(join(dir, changed[0].dir, "package.json"), "utf8"),
  );
  await $`git add .`;
  try {
