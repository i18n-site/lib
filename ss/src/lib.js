...
const createShadowsocksSocket = (会话密钥) => {
  ...
  const encrypt = async (data) => {
    ...
    return new Uint8Array(
      await crypto.subtle.encrypt({ name: "AES-GCM", iv: nonce }, 会话密钥, data),
    );
  };
  ...
  const decrypt = async (data) => {
    ...
    return new Uint8Array(
      await crypto.subtle.decrypt({ name: "AES-GCM", iv: nonce }, 会话密钥, data),
    );
  };
  ...
  const handleRemoteData = async (data) => {
    ...
    while (true) {
      ...
      const payload = await decrypt(remoteBuffer.slice(2 + 16, 2 + 16 + length + 16));
      duplex.push(payload);
