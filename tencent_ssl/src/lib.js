...
const bindCertToZone = async (teo, zone_id, host, cert_id) => {
...
export default (secretId, secretKey) => {
  ...
  return async (host, key, crt) => {
    const zones = (
      ...
      if (!is_active) {
        console.log(z.ZoneName, z.ActiveStatus);
      }
      return is_active;
    });

    if (zones.length > 0) {
      const cert_id = await uploadCert(
        ssl,
        `${host}-${new Date(new X509Certificate(crt).validFrom)
          .toISOString()
          .slice(0, 10)}`,
        key,
        crt,
