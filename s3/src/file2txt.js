...
const signRequest = async (access_key, secret_key, params) => {
  ...
  const signature = await crypto.subtle.sign("HMAC", key, encoder.encode(string_to_sign));
...
const callImmApi = async (access_key, secret_key, region, action, params) => {
  ...
  const query = Object.keys(common_params)
    .map((k) => `${encodeURIComponent(k)}=${encodeURIComponent(common_params[k])}`)
    .join("&");
...
const createDocConvertTask = async (access_key, secret_key, region, project, src_uri, tgt_uri) => {
  const result = await callImmApi(access_key, secret_key, region, "CreateOfficeConversionTask", {
    ProjectName: project,
    SourceURI: src_uri,
    TargetType: "txt",
...
const getDocConvertResult = async (access_key, secret_key, region, project, task_id) => {
...
const waitForTask = async (access_key, secret_key, region, project, task_id, interval = 1000) => {
  const result = await getDocConvertResult(access_key, secret_key, region, project, task_id);
  ...
  return waitForTask(access_key, secret_key, region, project, task_id, interval);
};

export default async (access_key, secret_key, region, project, bucket, filepath) => {
